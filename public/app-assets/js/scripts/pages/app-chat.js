...
$(function () {
    ...
    function getMessages(id) {
        $.ajax({
            url: "/admin/getusermessage/" + id,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
                $(".chats").html(data.messages);
                $(".chat-navbar").html(data.chatheader);
                userChats.animate(
...
function enterChat(source) {
    ...
    var sender_id = $("input[name=sender]").val();
    var userdp = $("input[name=userdp]").val();
    var chatuser = "chatuser" + sender_id;
    var msgcontent = '<div class="chat-content"><p>' + message + "</p></div>";
    var count = $("." + chatuser + ":last-child .chat-body").attr("data-count");
    var formdata = {
        sender: sender_id,
        receiver: receiver,
        message: msgcontent,
    ...
    if (/\S/.test(message)) {
        $.ajax({
            ...
            success: function (data) {
                if (count > 0) {
                    $(".chat:last-child .chat-body").append(msgcontent);
                } else {
                    msgcontent = makemsg(msgcontent, userdp, chatuser);
                    $(".chats").append(msgcontent);
                }
                $(".message").val("");
...
function makemsg(msgcontent, userdp, chatuser) {
    var msg = "";
    msg +=
    ...
    <div class="chat-body" data-count="1">
        ` +
        msgcontent +
        `
    </div>
